...
window.pre.onJobs(async (jobs) => {
  ...
  for (const job of jobs || []) {
    try {
      ...
      out.push({ skin: job.skin, file: job.file, changed: r.changed, dataUrl: r.changed ? r.dataUrl : null });
    } catch (e) {
      out.push({ skin: job.skin, file: job.file, error: String((e && e.message) || e) });
    }
  }
  window.pre.done(out);
});
