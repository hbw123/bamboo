...
一套皮肤就是一个「角色包」。开发时可以放在 `app/assets/skins/<名字>/`；安装版建议放在用户皮肤目录：

| 平台 | 用户皮肤目录 |
|---|---|
| macOS | `~/Library/Application Support/PandaPet/skins/<名字>/` |
| Windows | `%APPDATA%\PandaPet\skins\<名字>\` |
| Linux | `~/.local/share/PandaPet/skins/<名字>/` |

托盘菜单里点「打开皮肤文件夹」可以直达。用户皮肤和内置皮肤同名时，优先使用用户皮肤。
...
**加一套 = 新建这个文件夹、按下面的约定名放好图即可**，不用写任何清单文件。托盘「皮肤 ▸」
会自动列出，点选即时切换。内置 `default`（熊猫）一套。
...
**背景处理**（用户皮肤默认不自动抠底，避免误伤照片/白色主体）。三种做法，从省事到最稳：

1. **直接用透明 PNG** —— 最省事，什么都不用配。
2. **色键背景（推荐给非透明素材）** —— 作图时把背景涂成一个**角色里绝不会出现的纯色**
   （比如荧光品红 `#FF00FF`、荧光绿 `#00FF00`），在皮肤目录放 `manifest.json` 指定它，
   程序会精确抠掉那个颜色、**绝不误伤主体**（连白色衣服都不会被吃掉）：
   ...
   `tolerance` 可选（默认 60），颜色和背景越接近的边缘越干净就调小、抠不净就调大。
3. **自动猜浅色背景**（`"process": { "keyOut": true }`）—— 对"浅底 + 深色描边包住主体"的
   插画有效，但主体里的浅色区域可能被误伤，不如色键稳。内置皮肤走的就是这个（旧行为）。
...
自己的角色。这是开源共创的关键，模板见 [`skins/default/prompt.md`](app/assets/skins/default/prompt.md)。
...
├── app/                      # 展示层：Electron 桌宠
│   ├── assets/
│   │   ├── skins/            # 角色包（每套：约定命名的图 + lines + prompt）
│   │   └── sounds/           # 提示音
